...
vf22=slice_velnotp.txt
vf13=slice_veldifftp.txt
vf23=slice_veldiffnotp.txt
...
tail -$initvnum $initvf | awk '{print $2, -4.079+$1}' | gmt psxy -JX2.6c/-2.6c -R0/9/-4.079/21 -W1p -B2:Velocity\ \(km\/s\):/5:Depth\ \(km\):WseN -K -X2.5c -Y10c > $outf.ps
...
gmt grdimage $vf12.grd -JX5c/-2.6c -C$outf.cpt -R0/$xmax/-4.079/21 -B10:Distance\ \(km\):/5WseN -O -K -X6c -Y3.5c>> $outf.ps
gmt grdcontour $resf.grd -J -R -C$resln -W1p,black -S4 -O -K >> $outf.ps
awk '{print $1,-4.079+$3}' elev_line.txt | gmt psxy -J -R -W2p -O -K >>$outf.ps


# Part 2 2 (x,y)
awk '{print $1,-4.079+$3,100*$4}' "$vf22" | gmt surface -R0/$xmax/-5/25 -G$vf22.nm.grd -I$xyint -T0.25 -C0.1
gmt grdsample $vf22.nm.grd -G$vf22.rf.grd -I$xyrfint
gmt grdmath $vf22.rf.grd elev_poly.grd OR = $vf22.grd

gmt grdimage $vf22.grd -JX5c/-2.6c -C$outf.cpt -R0/$xmax/-4.079/21 -B10/5WseN -O -K -Y-3.5c>> $outf.ps
gmt grdcontour $resf.grd -J -R -C$resln -W1p,black -S4 -O -K >> $outf.ps
awk '{print $1,-4.079+$3}' elev_line.txt | gmt psxy -J -R -W2p -O -K >>$outf.ps
...
gmt grdimage $vf13.grd -JX5c/-2.6c -C$outf.cpt -R0/$xmax/-4.079/21 -B10:Distance\ \(km\):/5WseN -O -K -X6c -Y3.5c>> $outf.ps
awk '{print $1,-4.079+$3}' elev_line.txt | gmt psxy -J -R -W2p -O -K >>$outf.ps


# Part 2 3 (x,y)
awk '{print $1,-4.079+$3,100*$4}' "$vf23" | gmt surface -R0/$xmax/-5/25 -G$vf23.nm.grd -I$xyint -T0.25 -C0.1
gmt grdsample $vf23.nm.grd -G$vf23.rf.grd -I$xyrfint
gmt grd2xyz $vf23.rf.grd > temp2.txt

./valmask temp1.txt temp2.txt $vf23.mk.txt $tmpresl

gmt xyz2grd $vf23.mk.txt -G$vf23.mk.grd  -R0/$xmax/-5/25 -I$xyrfint
gmt grdmath $vf23.mk.grd elev_poly.grd OR = $vf23.grd
gmt grdimage $vf23.grd -JX5c/-2.6c -C$outf.cpt -R0/$xmax/-4.079/21 -B10/5WseN -O -K -Y-3.5c>> $outf.ps
awk '{print $1,-4.079+$3}' elev_line.txt | gmt psxy -J -R -W2p -O -K >>$outf.ps


gmt psscale -D-3.5c/-0.5c/12c/0.2ch -O -K -C$outf.cpt -B4:Velocity\ perturbation\ \(\%\): >> $outf.ps


gmt pstext -R0/20/0/20 -Jx1c -N -O -X-15c -Y-5c << EOF >> $outf.ps
3 8.1 10 0.0 0 CB 1
8 8.1 10 0.0 0 CB 1'
9 11.8 10 0.0 0 CB 1
14 11.8 10 0.0 0 CB 1'
15 11.8 10 0.0 0 CB 1
20 11.8 10 0.0 0 CB 1'
2.5 11.5 8 0.0 0 CB a)
8.5 11.5 8 0.0 0 CB b)
14.5.5 11.5 8 0.0 0 CB c)
2.5 7.9 8 0.0 0 CB d)
8.5 7.9 8 0.0 0 CB e)
14.5 7.9 8 0.0 0 CB f)
EOF
