...
real(kind=8) :: tmp1(ldu,nmax),tmp2(ldu,nmax),tmp3(ldu,nmax)
...
do im=1,nevn
    if(nrow(im) .gt. 0)then
        ...
        call dgesdd('All',nrow(im),4,a,lda,s,u,ldu,vt,ldvt,work,lwork,iwork,info)
        
        tmp1=matmul(u,transpose(u))
        tmp2=matmul(vt,transpose(vt))


        do ir=1,4
            do ic=1,4
                vt(ir,ic)=vt(ir,ic)*s(ir)
            end do
        end do

        tmp3(1:nrow(im),1:4)=matmul(u(1:nrow(im),1:4),vt(1:4,1:4))
