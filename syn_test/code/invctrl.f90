...
open(21,file='invctrl.inp',status='old')
read(21,*)outf
read(21,*)inpf
read(21,*)nodefile
read(21,*)smin,smax
...
write(30,*)edgenode
do iv=1,nnode
    if(sg(iv) .gt. smax .or. sg(iv) .lt. smin)then
        write(30,2000)node(iv)%x,node(iv)%y,node(iv)%z,node(iv)%t
    else
        vper=((1.0d0/sg(iv))-node(iv)%t)/node(iv)%t
        if(vper .lt. vpermax .and. vper .gt. vpermin)then
            write(30,2000)node(iv)%x,node(iv)%y,node(iv)%z,1.0d0/sg(iv)
