...
interface
    subroutine cacut(isr,source,receiver,evnum,vinp,vnum,node,nnode,nr,nl,layer,&
      ...
      &topoxy,topoz,vair,imethod,icoodnt)
        ...
        integer :: isr,nnode,vnum,nr,ttlgnum,tpxnum,tpynum,imethod,icoodnt
    end subroutine
end interface
...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_fw_regul2tp.inp',status='old')
read(21,*)datafile
read(21,*)nodefile
read(21,*)velfile
read(21,*)ns
...
! Read velocity model for fmm
open(25,file=velfile,status='old')
read(25,*)vnum
do iv=1,vnum
    read(25,*)v(iv,1),v(iv,2)
end do
close(25)
...
do is=1,ns
    ...
    call cacut(is,source(is),preceiver,pevnum,v,vnum,node,nnode,&
...
do ig=1,rttlgnum
    if(rtravelt(ig)%z .gt. topozbtm)then
        call detlayer(vnl,vnum,vinp(:,1),rtravelt(ig)%z)
        rv(ig)=1.0d0/linterp(1.0d0/vinp(vnl,2),1.0d0/vinp(vnl+1,2),&
                            &vinp(vnl,1),vinp(vnl+1,1),rtravelt(ig)%z)
    else
...
!----------------------------------------------------------------------
call detlayer(vnl,vnum,vinp(:,1),source%z)
vsr=1.0d0/linterp(1.0d0/vinp(vnl,2),1.0d0/vinp(vnl+1,2),&
                    &vinp(vnl,1),vinp(vnl+1,1),source%z)
...
do ig=1,ttlgnum
    if(travelt(ig)%z .gt. topozbtm)then
        call detlayer(vnl,vnum,vinp(:,1),travelt(ig)%z)
        v(ig)=1.0d0/linterp(1.0d0/vinp(vnl,2),1.0d0/vinp(vnl+1,2),&
                            &vinp(vnl,1),vinp(vnl+1,1),travelt(ig)%z)
    else
...
do ir=1,nr
    ...
    !gxnum=nint((maxx-minx)/gdx)+1
    !gynum=nint((maxy-miny)/gdy)+1
    !gznum=nint((maxz-minz)/gdz)+1
    !call frech_regular3d(node,path,ipth,fd,gdx,gdy,gdz,minx,&
    !    & maxx,miny,maxy,minz,maxz,gxnum,gynum,gznum)
    call frech_reg2sph3d2(node,path,ipth,fd,layer,nl,topoxy,topoz,&
