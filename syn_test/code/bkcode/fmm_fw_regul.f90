...
interface
    subroutine cacut(isr,source,receiver,evnum,vinp,vnum,node,nnode,nr,gdx,gdy,gdz,&
      ...
      &topoxy,topoz,vair,imethod,icoodnt)
        ...
        real(kind=8) :: topoz(maxgrid)
        real(kind=8) :: dx,dy,dz,gdx,gdy,gdz
...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_fw_regul.inp',status='old')
read(21,*)datafile
read(21,*)nodefile
read(21,*)velfile
read(21,*)ns
...
do is=1,ns
    ...
    call cacut(is,source(is),preceiver,pevnum,v,vnum,node,nnode,&
               &nrcver(is),gdx,gdy,gdz,dx,dy,dz,&
...
!--------------------------------------------------------------------------
subroutine cacut(isr,source,receiver,evnum,vinp,vnum,node,nnode,nr,gdx,gdy,&
  &gdz,dx,dy,dz,minx,maxx,miny,maxy,minz,maxz,ttlgnum,rfxint,rfyint,rfzint,&
  &rfxrg,rfyrg,rfzrg,tpminx,tpmaxx,tpminy,tpmaxy,tpxnum,tpynum,topozbtm,&
  &topoxy,topoz,vair,imethod,icoodnt)
...
do ig=1,rttlgnum
    !if(rtravelt(ig)%z .gt. topozbtm)then
        call vfrom1d(rv(ig),rtravelt(ig)%z,vinp,vnum)
    !else
...
!----------------------------------------------------------------------
call vfrom1d(vsr,source%z,vinp,vnum)
...
do ig=1,ttlgnum
    !if(travelt(ig)%z .gt. topozbtm)then
        call vfrom1d(v(ig),travelt(ig)%z,vinp,vnum)
    !else
...
do ir=1,nr
    ...
    call frech_regsph3d(node,path,ipth,fd,gdx,gdy,gdz,minx,&
