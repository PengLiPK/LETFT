...
type(tstrct3d) :: s3d(maxgrid3d)
type(tstrct) :: topoxy(maxgrid)
real(kind=8) :: topoz(maxgrid)
real(kind=8) :: s(maxgrd1d,3),v(maxgrd1d,3)
...
interface
    subroutine cacut(isr,source,receiver,evnum,sdx,sdy,sdz,s3d,node,nnode,nr,nl,layer,&
      ...
      &topoxy,topoz,vair,imethod,icoodnt)
        ...
        real(kind=8) :: layer(maxgrd1d,3)
        real(kind=8) :: topoz(maxgrid)
        real(kind=8) :: dx,dy,dz,sdx,sdy,sdz
...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_fw_regul3.inp',status='old')
read(21,*)datafile
read(21,*)nodefile
read(21,*)velfile
read(21,*)s3dfile
...
! Read 3d slowness file
open(22,file=s3dfile,status='old')
read(22,*)nv
do iv=1,nv
    read(22,*)s3d(iv)%x,s3d(iv)%y,s3d(iv)%z,s3d(iv)%t
    s3d(iv)%dxx=0d0
    s3d(iv)%num=iv
    s3d(iv)%stat=0
end do
...
do is=1,ns
    ...
    call cacut(is,source(is),preceiver,pevnum,sdx,sdy,sdz,&
               &s3d,node,nnode,&
               ...
               &tpynum,topozbtm,topoxy,topoz,sair,imethod,icoord)
...
2005 format(i5.5)
    ...
    tbypthname(is)="tbypth"//trim(sourcenum)//".txt"
...
call mergefile(tbypthname,ns,"tbypth.txt",0)
...
!--------------------------------------------------------------------------
subroutine cacut(isr,source,receiver,evnum,sdx,sdy,sdz,s3d,node,&
  &nnode,nr,nl,layer,dx,dy,dz,minx,maxx,miny,maxy,minz,maxz,ttlgnum,rfxint,&
  &rfyint,rfzint,rfxrg,rfyrg,rfzrg,tpminx,tpmaxx,tpminy,tpmaxy,tpxnum,tpynum,&
  &topozbtm,topoxy,topoz,sair,imethod,icoodnt)
...
type(tstrct3d) :: s3d(maxgrid3d)
type(tstrct3d) :: node(maxgrid3d)
type(tstrct3d), target :: sorig(maxgrid3d)
type(pstrct3d), pointer :: prs(:)
...
real(kind=8) :: sair
real(kind=8) :: tbyfd
real(kind=8) :: tmppathx,tmppathy,tmppathz,tmpaths,tmppatht,tmppathdist
...
character(len=70) :: tbyfdname
character(len=70) :: tbypthname
...
!------------------------------------------------------------------------
sorig=s3d
sxnum=nint((maxx-minx)/sdx)+1
synum=nint((maxy-miny)/sdy)+1
sznum=nint((maxz-minz)/sdz)+1
...
do ig=1,rttlgnum
    if(rtravelt(ig)%z .gt. topozbtm)then
        call localcood3d(rtravelt(ig)%x,rtravelt(ig)%y,rtravelt(ig)%z,&
                        &sorig,prs,sdx,sdy,sdz,minx,maxx,miny,maxy,minz,&
                        &maxz,sxnum,synum,sznum)
        rs(ig)=trilinear(prs(1)%p%t,prs(2)%p%t,&
                        &prs(3)%p%t,prs(4)%p%t,&
                        &prs(5)%p%t,prs(6)%p%t,&
                        &prs(7)%p%t,prs(8)%p%t,&
                        &prs(1)%p%x,prs(1)%p%y,prs(1)%p%z,&
                        &prs(8)%p%x,prs(8)%p%y,prs(8)%p%z,&
                        &rtravelt(ig)%x,rtravelt(ig)%y,rtravelt(ig)%z,0)
    else
...
!    write(230,3000)ig,rs(ig),rtravelt(ig)%x,rtravelt(ig)%y,rtravelt(ig)%z
...
!----------------------------------------------------------------------
call localcood3d(source%x,source%y,source%z,&
                &sorig,prs,sdx,sdy,sdz,minx,maxx,miny,maxy,minz,&
                &maxz,sxnum,synum,sznum)
ssr=trilinear(prs(1)%p%t,prs(2)%p%t,&
             &prs(3)%p%t,prs(4)%p%t,&
             &prs(5)%p%t,prs(6)%p%t,&
             &prs(7)%p%t,prs(8)%p%t,&
             &prs(1)%p%x,prs(1)%p%y,prs(1)%p%z,&
...
rtravelt(rcnum)%t=2.0d0*srdist/(ssr+rs(rcnum))
...
do ix=-1,1,2
    ...
    if((rgnum .ge. 1) .and. (rgnum .le. rttlgnum) .and.&
      &(dble(ix)*(rtravelt(rgnum)%x-rtravelt(rcnum)%x) .gt. 0d0))then
        ...
        rtravelt(rgnum)%t=2.0d0*srdist/(ssr+rs(rgnum))
...
tbypthname="tbypth"//trim(sourcenum)//".txt"
...
open(115,file=tbyfdname,status='replace')
open(120,file=tbypthname,status='replace')
pathrec=0
fdrec=0
do ir=1,nr
    ! Calculate travel time for one receiver
    call localcood3d(receiver(ir)%x,receiver(ir)%y,receiver(ir)%z,&
                  &travelt,prs,dx,dy,dz,minx,maxx,miny,maxy,minz,&
                  &maxz,xnum,ynum,znum)
    receiver(ir)%t=trilinear(prs(1)%p%t,prs(2)%p%t,prs(3)%p%t,&
                   &prs(4)%p%t,prs(5)%p%t,prs(6)%p%t,prs(7)%p%t,&
                   &prs(8)%p%t,prs(1)%p%x,prs(1)%p%y,prs(1)%p%z,&
                   &prs(8)%p%x,prs(8)%p%y,prs(8)%p%z,receiver(ir)%x,&
    ...
    call frech_reg2sph3d(node,path,ipth,fd,layer,nl)
    ...
    do ippth=1,nnode
        if(fd(ippth)/=0d0)then
            ...
            tbyfd=tbyfd+fd(ippth)/node(ippth)%t
        end if
    end do

    write(115,*)tbyfd,receiver(ir)%x,receiver(ir)%y,receiver(ir)%z
    ...
    tmppatht=0d0
    do ippth=1,ipth-1
        tmppathx=(path(ippth)%x+path(ippth+1)%x)/2.0d0
        tmppathy=(path(ippth)%y+path(ippth+1)%y)/2.0d0
        tmppathz=(path(ippth)%z+path(ippth+1)%z)/2.0d0
        !call localcood3d(tmppathx,tmppathy,tmppathz,&
        !              &node,prs,1.0d-1,1.0d-1,1.0d1,minx,maxx,miny,maxy,minz,&
        !              &6.0d1,8,6,7)
        call localcood3d(tmppathx,tmppathy,tmppathz,&
                        &sorig,prs,sdx,sdy,sdz,minx,maxx,miny,maxy,minz,&
                        &maxz,sxnum,synum,sznum)
        tmpaths=trilinear(prs(1)%p%t,prs(2)%p%t,&
                          &prs(3)%p%t,prs(4)%p%t,&
                          &prs(5)%p%t,prs(6)%p%t,&
                          &prs(7)%p%t,prs(8)%p%t,&
                          &prs(1)%p%x,prs(1)%p%y,prs(1)%p%z,&
                          &prs(8)%p%x,prs(8)%p%y,prs(8)%p%z,&
                          &tmppathx,tmppathy,tmppathz,0)
        !call vfrom1d(tmpaths0,tmppathz,vinp,vnum)
        !tmpaths=tmpaths0+tmppathdv
        call sphdist(path(ippth)%x,path(ippth)%y,path(ippth)%z,&
                    &path(ippth+1)%x,path(ippth+1)%y,path(ippth+1)%z,&
                    &tmppathdist)
        tmppatht=tmppatht+tmppathdist*tmpaths
    end do
    write(120,*)tmppatht,receiver(ir)%x,receiver(ir)%y,receiver(ir)%z
