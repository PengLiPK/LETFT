...
real(kind=8) :: rmax
real(kind=8) :: lambdaloc,lambdasl
...
open(22,file='invcgls3.inp')
read(22,*)FdFile
read(22,*)dwsfile,thrshd
read(22,*)datafile
read(22,*)oputmodel
read(22,*)idmax,immax,islmax
read(22,*)lambdaloc,lambdasl
read(22,*)cdval,cmval
close(22)
...
! Caculating Alpha(k)
    ...
    do im=1,imtmax
        if(im .le. immax-islmax)then
            AlphaDown2=AlphaDown2+p(im)*lambdaloc*cminv(im)*p(im)
        else
            AlphaDown2=AlphaDown2+p(im)*lambdasl*cminv(im)*p(im)
        end if
    end do
...
! Caculating m(k+1), r(k+1), Beta(k+1), will use Gp upstairs.
    ...
    do im=1,imtmax
        if(im .le. immax-islmax)then 
            r(im)=r(im)-Alpha*lambdaloc*cminv(im)*p(im)
        else
            r(im)=r(im)-Alpha*lambdasl*cminv(im)*p(im)
        end if
    end do
