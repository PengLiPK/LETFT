...
! vconst:
!       Assigned velocity of the deleted nodes on the edges of study area.
! teth, tethnum, it:
...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_synt3d.inp',status='old')
read(21,*)datafile
read(21,*)velfile
read(21,*)ns
read(21,*)vdx,vdy,vdz
...
do is=1,ns
    ...
    call cacut(is,source(is),preceiver,pevnid,vdx,vdy,vdz,&
               &vinp,nrcver(is),dx,dy,dz,&
...
!--------------------------------------------------------------------------
subroutine cacut(isr,source,receiver,evnid,vdx,vdy,vdz,vinp,&
  &nr,dx,dy,dz,minx,maxx,miny,maxy,minz,maxz,ttlgnum,rfxint,rfyint,rfzint,&
...
type(tstrct3d) :: vinp(maxgrid3d)
type(tstrct3d), target :: vorig(maxgrid3d)
...
!------------------------------------------------------------------------
vorig=vinp
vxnum=nint((maxx-minx)/vdx)+1
vynum=nint((maxy-miny)/vdy)+1
vznum=nint((maxz-minz)/vdz)+1
...
do ig=1,rttlgnum
    if(rtravelt(ig)%z .gt. topozbtm)then
        call localcood3d(rtravelt(ig)%x,rtravelt(ig)%y,rtravelt(ig)%z,&
                        &vorig,prv,vdx,vdy,vdz,minx,maxx,miny,maxy,minz,&
                        &maxz,vxnum,vynum,vznum)
        rv(ig)=1.0d0/trilinear(1.0d0/prv(1)%p%t,1.0d0/prv(2)%p%t,&
                              &1.0d0/prv(3)%p%t,1.0d0/prv(4)%p%t,&
                              &1.0d0/prv(5)%p%t,1.0d0/prv(6)%p%t,&
                              &1.0d0/prv(7)%p%t,1.0d0/prv(8)%p%t,&
                              &prv(1)%p%x,prv(1)%p%y,prv(1)%p%z,&
                              &prv(8)%p%x,prv(8)%p%y,prv(8)%p%z,&
...
call localcood3d(source%x,source%y,source%z,&
                &vorig,prv,vdx,vdy,vdz,minx,maxx,miny,maxy,minz,&
                &maxz,vxnum,vynum,vznum)
vsr=1.0d0/trilinear(1.0d0/prv(1)%p%t,1.0d0/prv(2)%p%t,&
                      &1.0d0/prv(3)%p%t,1.0d0/prv(4)%p%t,&
                      &1.0d0/prv(5)%p%t,1.0d0/prv(6)%p%t,&
                      &1.0d0/prv(7)%p%t,1.0d0/prv(8)%p%t,&
                      &prv(1)%p%x,prv(1)%p%y,prv(1)%p%z,&
...
2200 format(f11.7,1x,f9.5,1x,f8.5,1x,f5.3,1x,i6,1x,f9.5,1x,f8.5,1x,f6.3)
