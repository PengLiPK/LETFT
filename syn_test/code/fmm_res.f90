...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_res.inp',status='old')
read(21,*)datafile
read(21,*)velfile
read(21,*)v1dfile
read(21,*)ns
...
open(24,file=v1dfile,status='old')
read(24,*)nlayer
do iv=1,nlayer
    read(24,*)v1d(iv,1),v1d(iv,2),v1d(iv,3)
end do
...
do is=1,ns
    ...
    call cacut(is,source(is),preceiver,pevnid,vinp,nlayer,v1d,&
...
!--------------------------------------------------------------------------
subroutine cacut(isr,source,receiver,evnid,vinp,nlayer,v1d,nr,nl,layer,&
...
real(kind=8) :: vsr,rvtemp,drstemp
real(kind=8) :: dx,dy,dz
...
do ig=1,rttlgnum
    if(rtravelt(ig)%z .gt. topozbtm)then
        call vfrom1d(rvtemp,rtravelt(ig)%z,v1d,nlayer)
        ...
        drstemp=trilinear2(vorig(n(1))%t,vorig(n(2))%t,&
                         &vorig(n(3))%t,vorig(n(4))%t,&
                         &vorig(n(5))%t,vorig(n(6))%t,&
                         &vorig(n(7))%t,vorig(n(8))%t,&
                         &vorig(n(1))%x,vorig(n(1))%y,vorig(n(1))%z,&
                         &vorig(n(8))%x,vorig(n(8))%y,vorig(n(8))%z,&
                         &rtravelt(ig)%x,rtravelt(ig)%y,rtravelt(ig)%z,&
                         &topoxy,topoz,tpminx,tpmaxx,tpminy,tpmaxy,&
                         &tpxnum,tpynum,vair)
        rv(ig)=1.0d0/((1.0d0/rvtemp)+drstemp)
    else
...
!----------------------------------------------------------------------
call vfrom1d(rvtemp,source%z,v1d,nlayer)
call locatcood3d2(n,nl(1),layer(1:maxgrd1d,1),nl(2),&
                  &layer(1:maxgrd1d,2),nl(3),layer(1:maxgrd1d,3),&
...
vsr=1.0d0/((1.0d0/rvtemp)+drstemp)
...
do ig=1,ttlgnum
    if(travelt(ig)%z .gt. topozbtm)then
        call vfrom1d(rvtemp,travelt(ig)%z,v1d,nlayer)
        call locatcood3d2(n,nl(1),layer(1:maxgrd1d,1),nl(2),&
                          &layer(1:maxgrd1d,2),nl(3),layer(1:maxgrd1d,3),&
        ...
        drstemp=trilinear2(vorig(n(1))%t,vorig(n(2))%t,&
                         ...
                         &topoxy,topoz,tpminx,tpmaxx,tpminy,tpmaxy,&
                         &tpxnum,tpynum,vair)
        v(ig)=1.0d0/((1.0d0/rvtemp)+drstemp)
    else
