...
! Read parameters.
!-----------------------------------------------------------------------
open(21,file='fmm_fw_regul2rl.inp',status='old')
read(21,*)datafile
read(21,*)nodefile
read(21,*)ns
read(21,*)dx,dy,dz
...
do ir=1,nr
    ...
    slrc=trilinear(node(n(1))%t,node(n(2))%t,&
                   &node(n(3))%t,node(n(4))%t,&
                   &node(n(5))%t,node(n(6))%t,&
                   &node(n(7))%t,node(n(8))%t,&
                   &node(n(1))%x,node(n(1))%y,node(n(1))%z,&
                   &node(n(8))%x,node(n(8))%y,node(n(8))%z,&
                   &receiver(ir)%x,receiver(ir)%y,receiver(ir)%z,1)
    call frech_reg2sph3drl(node,path,ipth,evnid(ir),ttlevn,slrc,fd,&
                           &layer,nl)
