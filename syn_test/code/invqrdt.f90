program invqrdt


implicit none
integer,parameter :: mmax=50000,nmax=500,nb=64
integer,parameter :: lda=mmax,lwork=nmax+nb*mmax
real(kind=8) :: g(mmax,nmax)
real(kind=8) :: d(mmax)
real(kind=8) :: mfinal(nmax)
real(kind=8) :: work(lwork)
real(kind=8) :: rnorm,gtmp
integer :: metagzero(nmax)
integer :: metag(nmax,2)
integer :: i,j,ir,ic,status1,info
integer :: imt,im,idmax,immax,imtmax,izero,izeromax
integer :: rowstart,tmpmetag
character(len=70) :: dfile,gfile,metagf,outf
!external dnrm2
external dgels


open(31,file='invqrdt.inp',status='old')
read(31,*)dfile
read(31,*)metagf
read(31,*)gfile
read(31,*)outf
read(31,*)idmax,immax
close(31)

! Read data
open(36,file=dfile)
do i=1,idmax
    read(36,*)d(i)
end do

print *, 'Read data finished'

! Read g
metag=0
open(40,file=metagf,status='old')
rowstart=1
imt=0
izero=0
do im=1,immax
    read(40,*)tmpmetag
    if(tmpmetag .ne. 0)then
        imt=imt+1
        metag(imt,1)=tmpmetag
        metag(imt,2)=rowstart
        rowstart=rowstart+metag(imt,1)
    else
        izero=izero+1
        metagzero(izero)=im
    end if
end do
imtmax=imt
izeromax=izero
close(40)


open(43,file=gfile,status='old',form='unformatted',&
    &access='direct',recl=12)
g=0
i=0
do imt=1,imtmax
    do j=1,metag(imt,1)
        i=i+1
        read(43,rec=i,iostat=status1)gtmp,ir
        if(status1/=0)exit
        g(ir,imt)=gtmp
    end do
end do


!do i=1,m
!    print *, (A(i,j),j=1,n)
!end do
!print *, (b(i),i=1,m)
print *, 'Read g finished'

call dgels('No transpose',idmax,imtmax,1,g,lda,d,idmax,work,lwork,info)

print *, 'Calculate model finished'

mfinal=1d0
do izero=1,izeromax
    mfinal(metagzero(izero))=0d0
end do

imt=0
do im=1,immax
    if(mfinal(im) .ne. 0d0)then
        imt=imt+1
        mfinal(im)=d(imt)
    end if
end do

open(66,file=outf,status='replace')
do im=1,immax
    write(66,*)mfinal(im)
end do
close(66)

!print *, 'result=',(b(i),i=1,n)


!print *, work

!rnorm=dnrm2(M-N,B(N+1),1)
!print *, 'rnorm=',rnorm

stop
end

