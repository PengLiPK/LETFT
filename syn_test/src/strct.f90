...
!            ** frech_reg2sph3drl (calculate frechet derivative of non-uniform 
!                             regular grids and earthquakes, in spherical coordinates)
...
! If all surrounders of target node are above earth surface, let the 
! value of target node be a constant.
!------------------------------------------------------------------------
...
real(kind=8) :: xtmp(4),ytmp(4),ztmp,vair
...
if(nlowerp .eq. 0 .and. nupperp .eq. 0)then
    trilinear2=1.0d0/vair
else
...
! This subroutine finds Frechet Derivative for inverse with 3d regular 
! velocity grid and earthquakes, in spherical coordinates. 
! Grid is non-uniform.
!-----------------------------------------------------------------------
subroutine frech_reg2sph3drl(vel,path,ipath,evnid,ttlevn,&
                            &slevn,fd,layer,nl)
...
do i=1,ipath-1
    ...
    fd(n(1)+ttlevn*4)=length*(vel(n(8))%x-x)*(vel(n(8))%y-y)*(vel(n(8))%z-z)/a&
                    &+fd(n(1)+ttlevn*4)
    fd(n(2)+ttlevn*4)=length*(x-vel(n(1))%x)*(vel(n(8))%y-y)*(vel(n(8))%z-z)/a&
                    &+fd(n(2)+ttlevn*4)
    fd(n(3)+ttlevn*4)=length*(vel(n(8))%x-x)*(y-vel(n(1))%y)*(vel(n(8))%z-z)/a&
                    &+fd(n(3)+ttlevn*4)
    fd(n(4)+ttlevn*4)=length*(x-vel(n(1))%x)*(y-vel(n(1))%y)*(vel(n(8))%z-z)/a&
                    &+fd(n(4)+ttlevn*4)
    fd(n(5)+ttlevn*4)=length*(vel(n(8))%x-x)*(vel(n(8))%y-y)*(z-vel(n(1))%z)/a&
                    &+fd(n(5)+ttlevn*4)
    fd(n(6)+ttlevn*4)=length*(x-vel(n(1))%x)*(vel(n(8))%y-y)*(z-vel(n(1))%z)/a&
                    &+fd(n(6)+ttlevn*4)
    fd(n(7)+ttlevn*4)=length*(vel(n(8))%x-x)*(y-vel(n(1))%y)*(z-vel(n(1))%z)/a&
                    &+fd(n(7)+ttlevn*4)
    fd(n(8)+ttlevn*4)=length*(x-vel(n(1))%x)*(y-vel(n(1))%y)*(z-vel(n(1))%z)/a&
                    &+fd(n(8)+ttlevn*4)
...
return
end subroutine frech_reg2sph3drl
!-----------------------------------------------------------------------
